...
exports.getOne = async (req, res, next) => {
  try {
    ...
    await exports.list(
      ...
      { json: data => { singleData = data; return data; } }, 
      next
    );
    ...
    return success(res, singleData.data[0]);
  } catch (err) { next(err); }
};
...
exports.create = async (req, res, next) => {
  try {
    ...
    await withTransaction(async (tx) => {
      ...
      r1.input('fName', sql.NVarChar(100), s.first_name ? s.first_name.trim() : 'Unknown');
      r1.input('mName', sql.NVarChar(100), s.middle_name ? s.middle_name.trim() : null);
      r1.input('lName', sql.NVarChar(100), s.last_name ? s.last_name.trim() : null);
      r1.input('gen', sql.VarChar(20), s.gender || 'Male');
      r1.input('dob', sql.Date, s.date_of_birth && s.date_of_birth !== '' ? s.date_of_birth : null);
      ...
      r1.input('admDate', sql.Date, s.admission_date && s.admission_date !== '' ? s.admission_date : new Date());
      ...
      r1.input('photo', sql.NVarChar(sql.MAX), s.photo_url && s.photo_url !== '' ? s.photo_url : null);
      ...
      if (enrolment && enrolment.section_id && enrolment.academic_year_id) {
        ...
        await r2.query(`INSERT INTO enrolments (id,school_id,student_id,section_id,academic_year_id,roll_no,is_active) VALUES(NEWID(),@sid,@stId,@secId,@ayId,@roll,1)`);
      }
      ...
      if (guardians && Array.isArray(guardians)) {
        for (const g of guardians) {
          ...
          r3.input('fn', sql.NVarChar(255), g.full_name.trim());
          r3.input('ph', sql.NVarChar(50), g.phone || '0000000000');
      ...
      // 5. Audit Log (Recent Activity integration)
      try {
        const rLog = tx.request();
        rLog.input('lid', sql.UniqueIdentifier, uuidv4());
        rLog.input('sid', sql.UniqueIdentifier, schoolId);
        rLog.input('uid', sql.UniqueIdentifier, currentUserId || newId);
        rLog.input('act', sql.VarChar(50), 'STUDENT_ADDED');
        rLog.input('det', sql.NVarChar(sql.MAX), JSON.stringify({ studentName: `${s.first_name} ${s.last_name || ''}`.trim() }));
        await rLog.query(`
          INSERT INTO audit_logs (id, school_id, user_id, action_type, details, created_at)
          VALUES (@lid, @sid, @uid, @act, @det, GETUTCDATE())
        `);
      } catch (logErr) {
        console.warn('Student Audit Log Failed:', logErr.message);
      }
    });
  ...
  try {
    ...
    await withTransaction(async (tx) => {
      ...
      const fields = [
        ...
        'previous_school','tc_no','address_permanent','address_current',
        'city','state','pincode','photo_url','medical_conditions',
        'disabilities','extra_curricular','is_active'
      ];
      ...
      if (s.date_of_birth !== undefined) { 
        sets.push(`date_of_birth = @dob`); 
        r1.input('dob', sql.Date, s.date_of_birth && s.date_of_birth !== '' ? s.date_of_birth : null); 
      }
      if (s.admission_date !== undefined) { 
        sets.push(`admission_date = @admDate`); 
        r1.input('admDate', sql.Date, s.admission_date && s.admission_date !== '' ? s.admission_date : null); 
      }
      if (s.admission_no !== undefined) { 
        sets.push(`admission_no = @admNo`); 
        r1.input('admNo', sql.NVarChar(100), s.admission_no); 
      }
      ...
      for (const f of fields) {
        if (s[f] !== undefined) {
          ...
          if (typeof val === 'boolean') { 
            r1.input(f, sql.Bit, val ? 1 : 0); 
          } else { 
            r1.input(f, sql.NVarChar(sql.MAX), val ? String(val) : null); 
          }
        }
      }
      ...
      // 3. Update Guardians (Soft delete old, Insert new)
      if (guardians && Array.isArray(guardians)) {
        const r3 = tx.request();
        r3.input('id', sql.UniqueIdentifier, id);
        r3.input('sid', sql.UniqueIdentifier, schoolId);
        await r3.query(`UPDATE student_guardians SET deleted_at = GETUTCDATE(), updated_at = GETUTCDATE() WHERE student_id = @id AND school_id = @sid AND deleted_at IS NULL`);
        ...
        for (const g of guardians) {
          ...
          r4.input('fn', sql.NVarChar(255), g.full_name.trim());
          r4.input('ph', sql.NVarChar(50), g.phone || '0000000000');
