...
exports.create = async (req, res, next) => {
  try {
    const { schoolId } = req.user;
    const { 
      full_name, email, phone, gender, password, employee_code, 
      join_date, department, designation, qualification, 
      experience_years, date_of_birth, avatar_url 
    } = req.body;
    ...
    await withTransaction(async (tx) => {
      ...
      r1.input('fn', sql.NVarChar(255), full_name.trim());
      r1.input('em', sql.NVarChar(255), email.trim().toLowerCase());
      ...
      await r1.query(`
        INSERT INTO users (id, full_name, email, phone, gender, password, date_of_birth, avatar_url) 
        VALUES (@id, @fn, @em, @ph, @gen, @pw, @dob, @avatar)
      `);
      ...
      await r2.query(`
        ...
        VALUES (@id, @sid, @uid, @role, @emp, @join)
      `);
      ...
      await r3.query(`
        ...
        VALUES (@id, @sid, @uid, @dept, @des, @qual, @exp)
      `);
    });
...
exports.update = async (req, res, next) => {
  try {
    ...
    const { userId } = req.params;
    const { 
      full_name, phone, gender, employee_code, department, 
      designation, is_active, qualification, experience_years, 
    ...
    await withTransaction(async (tx) => {
      ...
      r1.input('fn', sql.NVarChar(255), full_name ? full_name.trim() : null);
      r1.input('ph', sql.NVarChar(50), phone || null);
      r1.input('gen', sql.VarChar(20), finalGender);
      r1.input('dob', sql.Date, date_of_birth || null);
      r1.input('avatar', sql.NVarChar(sql.MAX), avatar_url || null);
